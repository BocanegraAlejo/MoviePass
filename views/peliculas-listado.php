...
<div class="container"> 
    ...
    <section class="busqueda col-md-12">
        <form method="get" class="form-inline" action="<?php echo FRONT_ROOT?>Pelicula/getPeliculasActuales">
            ...
            <input type="hidden" value="<?=$id_sala?>" name="id_sala">
    ...
    <nav aria-label="Page navigation example">
        ...
        <?php if ($this->cantPaginas > 1) { ?>
            <li class="page-item  <?php if($this->pagActual==1) echo 'disabled'; ?>"><a class="page-link" href="<?= FRONT_ROOT ?>Pelicula/getPeliculasActuales?id_cine=<?=$id_cine?>&id_sala=<?=$id_sala?>&page=<?=$this->pagActual-1?>&genero=<?=$this->generoActual?>&fecha_ini=<?=$this->fecha_ini?>&fecha_fin=<?=$this->fecha_fin ?>">&laquo;</a></li>
                <?php
                for($x=$primera;$x<=$ultima;$x++)
                {
                    ?><li class="page-item <?php if($this->pagActual == $x) echo 'active'; ?>"><a class="page-link" href="<?= FRONT_ROOT ?>Pelicula/getPeliculasActuales?id_cine=<?=$id_cine?>&id_sala=<?=$id_sala?>&page=<?=$x?>&genero=<?=$this->generoActual?>&fecha_ini=<?=$this->fecha_ini?>&fecha_fin=<?=$this->fecha_fin ?>"><?=$x?></a></li><?php
                }
                if ($x <= $this->cantPaginas)
                ?>
            <li class="page-item  <?php if($this->pagActual==$this->cantPaginas) echo 'disabled'; ?>"><a class="page-link" href="<?= FRONT_ROOT ?>Pelicula/getPeliculasActuales?id_cine=<?=$id_cine?>&id_sala=<?=$id_sala?>&page=<?=$this->pagActual+1?>&genero=<?=$this->generoActual?>&fecha_ini=<?=$this->fecha_ini?>&fecha_fin=<?=$this->fecha_fin ?>">&raquo;</a></li>
